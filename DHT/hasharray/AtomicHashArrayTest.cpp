...
pair<KeyT, ValueT> createEntry(int i) {
  return pair<KeyT, ValueT>((std::hash<int>()(i)),(i + 3));
}
/*
template <
    class KeyT,
...
struct EqTraits {
  ...
  bool operator()(char* a, const std::string b) {
    return legalKey(a) && (strlen(a) == b.size()) &&
        (strncmp(a, b.c_str(), b.size()) == 0);
  }
};
...
struct HashTraits {
  ...
  size_t operator()(const std::string a) {
    size_t result = 0;
    for (const auto& ch : a) {
...
struct KeyConvertTraits {
  ...
  char* operator()(const std::string a) { return strndup(a.c_str(), a.size()); }
};
...
{
  MmapAllocator<int> *m = new MmapAllocator<int> ();
  MmapAllocator<std::atomic<int>> *mutexes = new MmapAllocator<std::atomic<int>> ();

  int num_elements = 100;
  int *vec = m->allocate(num_elements);
  std::atomic<int> *at_m = mutexes->allocate(num_elements);

  for(int i=0;i<num_elements;i++)
	 vec[i] = 100;

  typedef AtomicHashArray<int32_t,int32_t,std::hash<int32_t>,std::equal_to<int32_t>,MmapAllocator<char>,AtomicHashArrayLinearProbeFcn> ah;

  auto am = ah::create(100);
  //testMap<int64_t,int64_t,MmapAllocator<char>,AtomicHashArrayQuadraticProbeFcn>();

  auto e = createEntry<int32_t,int32_t>(1);
  std::cout <<" f = "<<e.first<<" s = "<<e.second<<std::endl;
  auto ret = am->insert(e);
  ...
  if(pid==0)
  {
	    ...
	    auto e = createEntry<int32_t,int32_t>(id+2);
  	    std::cout <<" id = "<<id<<" f = "<<e.first<<" s = "<<e.second<<std::endl;
  	    auto ret = am->insert(e);
	 ...
	 /* int cn, n;
	  ...
	  for(int i=0;i<100;i++)
	  {
		do
		{
		   ...
		   cn = 0;
		   n = 1;
		}while(!(flag=at_m[i].compare_exchange_strong(cn,n,std::memory_order_seq_cst)));
  ...
  else
  {
    ...
    int wstatus;
	 ...
	 std::cout <<" size = "<<am->size()<<std::endl;
     mutexes->deallocate(at_m,num_elements);
     m->deallocate(vec,num_elements);
  }
}
