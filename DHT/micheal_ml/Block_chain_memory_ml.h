...
class BlockMap_ml
{
   ...
   private :
	...
	{
	    uint32_t hashval = HashFcn()(k);
	    return hashval % maxSize;
	}
  public:

	BlockMap_ml(uint32_t n,memory_pool<KeyT,ValueT,HashFcn,EqualFcn> *m,uint32_t s) : maxSize(n), pl(m),sub_table_size(s)
	{
  	   assert (maxSize > 0);
	   ...
	   for(int i=0;i<maxSize;i++)
	   {
	      allocated[i].store(0);
	      removed[i].store(0);
	      table[i].table = new blocktable_type(1,pl,log2(maxSize));
	   }
